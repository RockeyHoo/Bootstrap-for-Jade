...
dpUploader.SWFSettings = function(upload_url,flash_url,post_params,cnf,dp){
    ...
    this.button_text = "<span class='button'>选择图片</span>";
    this.button_text_style = '.button {color:#666666; font-size: 13px; font-weight: bold; text-align:center;}',
    ...
    this.button_image_url = "/s/css/theme/img/blank.png";
...
dpUploader.prototype.templates = {
    "upButton":"<div class='dpuploader-btn-container'><input type='button' value='浏览' /></div>",
...
dpUploader.prototype.create = function(){
    ...
    for(var i = 0; i<this.dpUploaderCtrls.count;i++){
        var params = null;
        if(this._postParams instanceof Array){
            params = this._postParams[i];
        }else{
            params = this._postParams;
        }
        var settings = new dpUploader.SWFSettings(this._uploadUrl,this._flashUrl,params,this._cnf,this);
...
dpUploader.prototype.onUploadError = function(file,errorCode,msg){
    console.log("error, code :%s, msg :%s",errorCode,msg);
    var pbCntDom = this.customSettings.dpuploader._getPbCntDom(this.customSettings.seq);
...
dpUploader.prototype.onUploadComplete = function(file){
...
dpUploader.prototype.onUploadSuccess = function(file,data,response){
	...
	fn.call(this.customSettings.dpuploader,data,this.customSettings.seq,file);
    }
}
